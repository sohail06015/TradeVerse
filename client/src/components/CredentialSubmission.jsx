...
const CredentialSubmission = ({ onClose, listing }) => {
  ...
  const handleSubmission = async (e) => {
    ...
    try{
      ...
      {credential, listingId: listing.id  }, { headers: {Authorization:
        `Bearer ${token}`} }   )
  ...
  return (
    ...
    >
      <div
        className="bg-white sm:rounded-lg shadow-2xl w-full max-w-lg
    h-screen sm:h-[320] flex flex-col"
      >
        {/* header */}
        <div
       ...
       text-white p-4 sm:rounded-t-lg flex items-center justify-between"
        >
          <div className="flex-1 min-w-0">
            ...
            <p>
              Adding Credential for {listing?.username} on {listing?.platform}
            </p>
          </div>
          <button
            onClick={onClose}
      ...
      gap-4 p-4 overflow-y-scroll"
        ...
        items-center gap-2">
              ...
              />
              <X className="w-5 h-5 text-gray-300 rounded outline-indigo-700
              cursor-pointer"  onClick={()=> setCredential((prev)=>prev.filter
              (( _, i)=> i !== index))}  />
            </div>
            );
          })}
