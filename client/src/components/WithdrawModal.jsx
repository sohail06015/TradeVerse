   ...
   <div className="bg-gradient-to-r from-indigo-600 to-indigo-400
     ...
     <form onSubmit={handleSubmission}  className='flex flex-col items-start
      ...
      <div className='grid grid-cols-[2fr_3fr_1fr] items-center gap-2 ' >
          Amount <input  onChange={(e) => setAmount(e.target.value)} value={amount} type= 'number'  className='w-full px-2
          py-1.5 text-sm border border-gray-300 rounded outline-indigo-400 ' 
           required />
      </div>
      ...
      {account.map((field, index)=>(
        ...
        setAccount((prev)=>prev.map((c, i)=> (i===index ? {
          ...c,value: e.target.value
        } : c )) )} className='w-full px-2
          py-1.5 text-sm border border-gray-300 rounded
           outline-indigo-400 '  />
