...
export const getDashboard = async (req, res) => {
    try {
        ...
        return res.json({
            dashboardData: {
                totalListings, totalRevenue,
                activeListings, totalUser, recentListings
            }
        })
...
// controller for getting all unverified listing with credential submitted
export const getAllUnverifiedListing = async (req, res) => {
    try {
        const listings = await prisma.listing.findMany({
            where: {
...
export const getAllTransactions = async (req, res) => {
    try {
        ...
        // get customer details for each transaction and add it to the
        // transaction object
...
// controller for withdraw requests
...
export const markWithdrawalAsPaid = async (req, res) => {
    try {
        ...
        if (withdrawal.isWithdrawn) {
            return res.status(404).json({ message: "Withdrawal already marked as paid" })
        }
